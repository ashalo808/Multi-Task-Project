...
class ClassificationTask(BaseTask):
    ...
    def initialize(self):
        ...
        # 设置最大序列长度
        self.max_seq_length = self.task_config.get("max_seq_length", 128)
    ...
    def _load_label_map(self) -> Dict[str, int]:
        ...
        if os.path.exists(label_map_path):
            ...
            # 如果没有找到标签映射文件，尝试从训练数据中提取
            try:
                train_data = self.load_json_data(self.get_dataset_path("train"))
                labels = sorted(list(set([item.get("label") for item in train_data])))
                return {label: idx for idx, label in enumerate(labels)}
            except Exception as e:
                raise ValueError(f"无法加载或创建标签映射: {e}")
    ...
    def preprocess_data(self, split: str) -> Dataset:
        ...
        # 筛选特定任务类型的数据
        task_type = self._get_task_type()  # 如'tnews', 'ocnli', 'chid'
        ...
        return ClassificationDataset(texts, labels, self.tokenizer, self.max_seq_length)
